"""Perform split of HWD+ dataset into train/val/test for classification task"""
...
def main():
    ...
    for split, idxs in tqdm(split_idxs.items(), desc="Performing data split"):
        ...
        for i, (image, label) in tqdm(
            enumerate(zip(split_images, split_labels)), total=len(split_labels), desc=split
        ):
